...
def main():
  ...
  time_mappings = {'1': 'one', '2': 'two', '3': 'three', '4': 'four', '5': 'five', \
                   '00': '', '10': 'ten', '15': 'fifteen', '30': 'thirty', '45': 'forty-five'}
  ...
  results = []
  for v in appointments_dict.values():
    hour, mins = v.split(":")
    if (hour not in time_mappings.keys() or mins not in time_mappings.keys()):
      results.append('unknown')
    else:
      time = time_mappings[hour] + " " + time_mappings[mins]
      results.append(time)
